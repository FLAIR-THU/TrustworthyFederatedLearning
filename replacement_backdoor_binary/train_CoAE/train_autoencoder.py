...
from models.autoencoder import AutoEncoder, AutoEncoder2, AutoEncoder3, AutoEncoder_extend
...
def train_batch(model, optimizer, code_y, hyperparameter_dict):
    ...
    # print("code_y", code_y)
    # =================== forward =====================
    code_y_hat, code_D_y = model(code_y) # decoder(encoder(code_y)), encoder(code_y)

    label_y_hat = torch.argmax(code_y_hat, dim=1) # ^y = decoder(encoder(y))
    label_D_y = torch.argmax(code_D_y, dim=1) # ~y = encoder(y)
    # print("code_D_y", code_D_y)
    loss_e = entropy(code_D_y) # lambda2
    loss_p = cross_entropy_for_onehot(code_y_hat, code_y) # original
    # loss_p = criterion(code_y_hat, label_y)
    loss_n = cross_entropy_for_onehot(code_D_y, code_y) # lambda1
...
if __name__ == '__main__':
    ...
    model = AutoEncoder_extend(input_dim=num_classes, encode_dim=2+num_classes * 6).to(device)
