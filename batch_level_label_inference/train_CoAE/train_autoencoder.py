...
def train_batch(model, optimizer, code_y, hyperparameter_dict):
    ...
    label_y_hat = torch.argmax(code_y_hat, dim=1) # decoder(encoder(y))
    label_D_y = torch.argmax(code_D_y, dim=1) # encoder(y)
    # print("code_D_y", code_D_y)
    loss_e = entropy(code_D_y) # for confustion
    loss_p = cross_entropy_for_onehot(code_y_hat, code_y) # for decoder acc
    ...
    for epoch in range(0, epochs + 1):
        ...
        for batch_start_idx in range(0, code_y.shape[0], batch_size):
            ...
            if (iteration + 1) % 20 == 0:
                ...
                # print("test_code_D_y: \n", test_code_D_y[0])
                tst_label_y_hat = torch.argmax(test_code_y_hat, dim=1) # decoder(encoder(y))
                tst_label_D_y = torch.argmax(test_code_D_y, dim=1) # decoder(y)
