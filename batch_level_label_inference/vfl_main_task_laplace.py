...
if __name__ == '__main__':
    ...
    if args.dataset_name == "cifar100":
        ...
        test_dst = NUSWIDEDataset('./data/NUS_WIDE', 'test') # num_classes should be changed manually
    ...
    ################## exps #################
    path = f'./exp_result/{args.dataset_name}/laplace/'
    if not os.path.exists(path):
        os.makedirs(path)
    path += 'main_task_acc.txt'
    num_exp = 10
    dp_list = [0.00005, 0.0001, 0.0005, 0.001, 0.01]
    ...
    for dp_strength in dp_list:
        ...
        append_exp_res(path, str(dp_strength) + ' ' + str(np.mean(test_acc_list)) + ' ' + str(np.max(test_acc_list)))
