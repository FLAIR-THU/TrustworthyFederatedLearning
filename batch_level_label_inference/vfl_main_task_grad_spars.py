...
if __name__ == '__main__':
    ...
    parser.add_argument('--apply_grad_spar', default=True, type=bool, help='whether to use gradient sparsification')
    ...
    ################## exps #################
    path = f'./exp_result/{args.dataset_name}/grad_spars/'
    if not os.path.exists(path):
        os.makedirs(path)
    path += 'main_task_acc.txt'
    num_exp = 10
    grad_spars_list = [90, 95, 96, 97, 98, 99]

    for grad_spars in grad_spars_list:
        test_acc_list = []
        for i in range(num_exp):
            args.grad_spars = grad_spars
            print(f'num_exp:{i + 1}, epochs:{args.epochs}, batchsize:{args.batch_size}, grad_spars:{grad_spars}')
            vfl_defence_image = VFLDefenceExperimentBase(args)
            test_acc = vfl_defence_image.train()
            test_acc_list.append(test_acc)
        append_exp_res(path, str(grad_spars) + ' ' + str(np.mean(test_acc_list)) + ' ' + str(np.max(test_acc_list)))
