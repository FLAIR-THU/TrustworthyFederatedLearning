...
struct XGBoostParty : Party
{
     ...
     * @return vector<vector<tuple<vector<float>, vector<float>, float, vector<float>>>>
     */
    vector<vector<tuple<vector<float>, vector<float>, float, vector<float>>>> greedy_search_split_from_pointer(vector<vector<float>> *gradient,
                                                                                                               vector<vector<float>> *hessian,
                                                                                                               vector<float> *y,
                                                                                                               vector<int> &idxs)
    {
            ...
            for (int p = 0; p < percentiles.size(); p++)
            {
                ...
                for (int r = current_min_idx; r < not_missing_values_count; r++)
                {
                    if (x_col[r] <= percentiles[p])
                    {
                        for (int c = 0; c < grad_dim; c++)
                        {
                            temp_grad[c] += gradient->at(idxs[x_col_idxs[r]]).at(c);
                            temp_hess[c] += hessian->at(idxs[x_col_idxs[r]]).at(c);
                        }
                        temp_left_size += 1.0;
