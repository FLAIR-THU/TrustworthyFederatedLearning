...
def plot_per_dataset_and_model(target_dataset, target_model, exp_dir, target_dir, filter_dict={'test_loss': 3, 'backdoor_acc':70}):
    ...
    for item in os.listdir(exp_dir):
        ...
        if os.path.isdir(sub_dir):
            ...
            if dataset[:5] == target_dataset[:5] and model == target_model and amplify_rate_output==1:
...
def plot_per_dataset_and_model_defense(target_dataset, target_model, exp_dir, target_dir, target_method='laplace'):
    ...
    for item in os.listdir(exp_dir):
        ...
        if os.path.isdir(sub_dir):
            ...
            if dataset[:5] == target_dataset[:5] and model == target_model and amplify_rate_output==1 and defense_model == target_method:
                print(os.path.join(sub_dir, 'log.txt'))
...
def save_defense_data(target_dataset, target_model, exp_dir):
    ...
    for item in os.listdir(exp_dir):
        ...
        if os.path.isdir(sub_dir):
            ...
            # else:
            #     autoencoder_coef = 0
            if dataset[:5] == target_dataset[:5] and model == target_model: #?? and amplify_rate==10
...
def plot_per_dataset_and_model_for_paper(target_dataset, target_model, exp_dir, target_dir):
    ...
    for item in os.listdir(exp_dir):
        ...
        if os.path.isdir(sub_dir):
            ...
            # filter condition
            if dataset[:5] == target_dataset[:5] and model == target_model and amplify_rate_output == 10:
                print(os.path.join(sub_dir, 'log.txt'))
                res = read_log(os.path.join(sub_dir, 'log.txt'))
                # remove unstable results
                valid_flag = True
...
def plot_scatter_plot_for_paper(input_file, target_dir, marker=False):
    ...
    for key in data:
        ...
        if defense_method in ['gaussian', 'laplace']:
            ...
            if 'nuswide' in input_file:
                if defense_param not in ['0.5', '1.0', '2.0']:
                    continue
            else:
                print("here, and defense_param is", defense_param)
    ...
    # color_list = ['#1f77b4', '#ff7f0e', '#2ca02c', '#9467bd', '#d62728', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf'] # change color[3] and color[4]
    ...
    for i, key in enumerate(x):
        ...
        ax.plot(x[key], y[key], '--', color=color_list[i], linewidth=2)
        if marker:
    ...
    if 'mnist' in input_file:
        ...
        ax.scatter([50.5],[95.0], label='w/o defense', marker='s', color='k', s=60)
...
def CoAE_distribution(target_dataset, target_model, exp_dir):
    ...
    for item in res_to_plot:
        if item == "mnist-mlp2-1-autoencoder-0-0.0-0.0-0.5":
            ref = res_to_plot[item]['raw']
            print(type(ref),len(ref),len(ref[0]))
            backdoor_acc_list = []
            main_acc_list = []
...
if __name__ == '__main__':
    ...
    # dataset = 'mnist'
    # model = 'mlp2'
    # exp_dir = './experiment_plot_defense'
    dataset = 'cifar20'
    dataset = 'cifar100'
    model = 'mlp2'
    exp_dir = './experiment_plot_50_defense'
    exp_dir = './experiment_plot_model_DSGD_50_defense'
    # exp_dir = './experiment_plot_model_DSGD2_50_defense'
