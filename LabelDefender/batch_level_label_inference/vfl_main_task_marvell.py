...
if __name__ == '__main__':
    ...
    ################## exps #################
    path = f'./exp_result/{args.dataset_name}/marvell/'
    if not os.path.exists(path):
        os.makedirs(path)
    path += 'main_task_acc.txt'
    num_exp = 10
    
    marvell_s_list = [1, 2, 5, 10]

    for s in marvell_s_list:
        test_acc_list = []
        for i in range(num_exp):
            args.marvell_s = s
            print(f'num_exp:{i + 1}, epochs:{args.epochs}, batchsize:{args.batch_size}, marvell_s:{args.marvell_s}')
            vfl_defence_image = VFLDefenceExperimentBase(args)
            test_acc = vfl_defence_image.train()
            test_acc_list.append(test_acc)
        append_exp_res(path, str(s) + ' ' + str(np.mean(test_acc_list))+ ' ' + str(test_acc_list) + ' ' + str(np.max(test_acc_list)))
