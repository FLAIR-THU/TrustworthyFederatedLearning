...
if __name__ == '__main__':
    ...
    if args.apply_ppdl:
        path = path + 'ppdl/'
        parameter = args.ppdl_theta_u
    elif args.apply_gc:
        path = path + 'gradient_compression/'
        parameter = args.gc_preserved_percent
    elif args.apply_lap_noise:
        path = path + 'laplase_noise_gradient/'
        parameter = args.noise_scale
    elif args.apply_discrete_gradients:
        path = path + 'discrete_gradients/'
        parameter = args.discrete_gradients_bins
    elif args.apply_laplace:
        path = path + 'laplace/'
        parameter = args.dp_strength
    if not os.path.exists(path):
        os.makedirs(path)
    path = path + 'main_task_acc.txt'
    num_exp = 10
    ...
    for i in range(num_exp):
        print(f'num_exp:{i + 1}, epochs:{args.epochs}, batchsize:{args.batch_size}, parameter:{parameter}')
        vfl_defence_image = VFLDefenceExperimentBase(args)
        test_acc, test_acc_topk = vfl_defence_image.train()
        test_acc_list.append(test_acc)
        test_acc_topk_list.append(test_acc_topk)
    append_exp_res(path, str(parameter) + ' ' + str(np.mean(test_acc_list))+ ' ' + str(test_acc_list) + ' ' + str(np.max(test_acc_list)) + ' | ' + str(np.mean(test_acc_topk_list))+ ' ' + str(test_acc_topk_list) + ' ' + str(np.max(test_acc_topk_list)))
